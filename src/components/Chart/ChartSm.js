...
export function ChartSm({ results, coloris }) {
  const { socialForm, CA, charges, cotisations, revenuNet } = results;

  const smallScreen = (value) => {
    return value * 0.9;
  };
    ...
    if (results.CA) {
      ...
      setOptions(() => {
        ...
        annotations.revenuNet = {
          ...
          label: {
            display: true,
            yAdjust: smallScreen(isExpenseLow ? 28 : -27),
            xAdjust: smallScreen(-118),
        ...
        annotations.chiffreDaffaires = {
          ...
          display: true,
          label: {
            xAdjust: smallScreen(-113),
            yAdjust: smallScreen(-27),
            display: true,
        ...
        return {
          ...
          scales: {
            ...
            y: {
              stacked: true,
              display: false,
              max: 1.18 * (revenuNet < 0 ? charges + cotisations : CA),
              min: -0.03 * Math.max(CA, cotisations),
            },
          },
          plugins: {
            tooltip: {
